...
export function ExpenseFormDialog({
    ...
    const [initialType, setInitialType] = useState("simple");
    ...
    useEffect(() => {
        if (open) {
            ...
            if (editTarget) {
                newType = editTarget.type || "simple";
                newForm = {
            ...
            setForm(newForm);
            setInitialForm(JSON.stringify({ type: newType, form: newForm }));
            setErrors({});
            setProcessing(false);
    ...
    const hasChanges = () => {
        ...
        const current = JSON.stringify({ type, form });
        return current !== initialForm;
    };
    ...
    const handleSubmit = () => {
        ...
        } else if (type === "store_transfer_in") {
            if (!form.amount || amountNum <= 0) {
            ...
            <Dialog open={open} onOpenChange={handleClose}>
                ...
                >
                    <DialogHeader>
                        ...
                        >
                            {isEdit
                                ? "Edit data transaksi yang sudah dicatat"
                        ...
                        <div className="bg-slate-50 border rounded-lg p-2.5 sm:p-3">
                            ...
                            {type === "store_transfer_in" && (
                                <p className="text-xs text-emerald-600 mt-1">
                                    Transfer akan menambah saldo kas toko dan
                                    mengurangi saldo dompet owner.
                                </p>
                            )}
                        </div>
